...
class block_quicklinks_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        $mform->addElement('text', 'config_number_of_links', get_string('numberoflinks', 'block_quicklinks'));
        ...
        for ($i = 1; $i <= $number_of_links; $i++) {
            $mform->addElement('text', 'config_link_title_' . $i, get_string('linktitle', 'block_quicklinks', $i));
            $mform->setType('config_link_title_' . $i, PARAM_TEXT);

            $mform->addElement('text', 'config_link_url_' . $i, get_string('linkurl', 'block_quicklinks', $i));
